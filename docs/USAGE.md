...
| `OPENOFFENSIVE_LLM_MODE` | `auto` | `auto`/`llm` run model-driven agents (a key is required — a scan without one errors); `scripted` runs the no-key demo playbook. The `--mode` flag overrides this per scan. |
...
OPENOFFENSIVE_SANDBOX=local openoffensive scan
```

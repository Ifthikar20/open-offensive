...
class ReconAgent(BaseAgent):
    ...
    def scripted(self) -> None:
        ...
        home = self.ctx.run(f"curl -s -i {self.target}/")
        banner = ""
        ...
        if banner:
            self._file(title="Server version disclosure", severity="info", endpoint="/",
                       evidence=f"Server header reveals '{banner}'.",
                       remediation="Suppress or genericise the Server header.", cwe="CWE-200")
        js = self.ctx.run(f"curl -s {self.target}/static/app.js")
        if "sk_live_" in js.stdout:
        ...
        if self.workspace_path:
            grep = self.ctx.run(
                f"grep -rEn 'sk_live_|api_key|password' {self.workspace_path} | head -20")
...
class InjectionAgent(BaseAgent):
    ...
    def scripted(self) -> None:
        ...
        sqli = self.ctx.run(f"curl -s -i \"{self.target}/login?user=admin%27&pass=x\"")
        if "SQL" in sqli.stdout or "SQLException" in sqli.stdout:
        ...
        xss = self.ctx.run(f"curl -s \"{self.target}/search?q=<script>xss1()</script>\"")
...
class AccessAgent(BaseAgent):
    ...
    def scripted(self) -> None:
        ...
        walk = self.ctx.run(
            f"for i in 1 2 3; do curl -s {self.target}/api/user/$i; echo; done")
